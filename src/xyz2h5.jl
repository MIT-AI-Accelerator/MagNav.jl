...
function par(val::SubString{String})
    return val == "*" ? NaN : parse(Float64, val)
end # function par
...
function remove_extension(data_file::String, extension::String)
    ...
    l = length(e)
    length(f) <= l && (return f)
    return f = lowercase(f[(end-l+1):end]) == lowercase(e) ? f[1:(end-l)] : f
...
function add_extension(data_file::String, extension::String)
    ...
    return f = lowercase(f[(end-l+1):end]) == lowercase(e) ? f : f*e
...
function field_check(s, t::Union{DataType,UnionAll})
    ...
    return [fields[i] for i in findall([getfield(s, f) isa t for f in fields])]
