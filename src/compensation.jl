...
function nn_comp_1_train(x, y, no_norm              = falses(size(x, 2));
                         ...
                         λ_sgl                = 0,
                         k_pca::Int           = -1,
                         data_norms::Tuple    = (zeros(Float32, 1, 1), zeros(Float32, 1, 1), zeros(Float32, 1, 1), zeros(Float32, 1, 1), zeros(Float32, 1, 1), [0.0f0], [0.0f0]),
                         model::Chain         = Chain(),
                         l_segs::Vector       = [length(y)],
                         x_test::Matrix       = Matrix{eltype(x)}(undef, 0, 0),
                         y_test::Vector       = Vector{eltype(y)}(undef, 0),
    ...
    if epoch_lbfgs > 0 # LBFGS, may overfit depending on iterations
        ...
        function lbfgs_train!(s_l, data_l, iter, silent)
            ...
            return optimize(only_fg!(fg!), p0, opt, options)
...
"""
function TL_vec2mat(TL_coef::Vector, terms; Bt_scale=50000.0f0)
    @assert any([:permanent, :p, :permanent3, :p3] .∈ (terms,)) "permanent terms are required"
    @assert any([:induced, :i, :induced6, :i6, :induced5, :i5, :induced3, :i3] .∈ (terms,)) "induced terms are required"
    @assert !any([:fdm, :f, :fdm3, :f3, :bias, :b] .∈ (terms,)) "derivative & bias terms may not be used"
    ...
    elseif any([:induced5, :i5] .∈ (terms,))
        TL_coef_i = [TL_coef[4] TL_coef[5]/2 TL_coef[6]/2
                     TL_coef[5]/2 TL_coef[7] TL_coef[8]/2
                     TL_coef[6]/2 TL_coef[8]/2 0.0f0] / Bt_scale
    elseif any([:induced3, :i3] .∈ (terms,))
        TL_coef_i = [TL_coef[4] 0.0f0 0.0f0
                     0.0f0 TL_coef[5] 0.0f0
                     0.0f0 0.0f0 TL_coef[6]] / Bt_scale
    end
    ...
    elseif any([:eddy8, :e8] .∈ (terms,))
        TL_coef_e = [TL_coef[N-7] TL_coef[N-6] TL_coef[N-5]
                     ...
                     TL_coef[N-1] TL_coef[N-0] 0.0f0] / Bt_scale
    elseif any([:eddy3, :e3] .∈ (terms,))
        TL_coef_e = [TL_coef[N-2] 0.0f0 0.0f0
                     0.0f0 TL_coef[N-1] 0.0f0
                     0.0f0 0.0f0 TL_coef[N-0]] / Bt_scale
    else
        TL_coef_e = []
    end
...
"""
function TL_mat2vec(TL_coef_p, TL_coef_i, TL_coef_e, terms; Bt_scale=50000.0f0)
    if any([:induced, :i, :induced6, :i6] .∈ (terms,))
...
function comp_train(comp_params::CompParams, xyz::XYZ, ind,
    ...
    if !isempty(ind_test)
        ...
        y_test = get_y(xyz_test, ind_test, map_val;
                       ...
                       sub_diurnal = sub_diurnal,
                       sub_igrf    = sub_igrf)
        (A_test, Bt_test, B_dot_test) = create_TL_A(getfield(xyz_test, use_vec), ind_test;
                                                    terms=terms_A, return_B=true)
    end
    ...
    if drop_fi
        ...
        for i in axes(x, 2)
            ...
            # train model
            if model_type in [:m1]
                (model, data_norms, y_hat_fi, err_fi) = nn_comp_1_train(x_fi, y, no_norm;
            ...
            elseif model_type in [:m2a, :m2b, :m2c, :m2d]
                (model, TL_coef, data_norms, y_hat_fi, err_fi) = nn_comp_2_train(A, x_fi, y, no_norm;
                                                                                 model_type  = model_type,
            ...
            elseif model_type in [:m3tl, :m3s, :m3v, :m3sc, :m3vc, :m3w, :m3tf]
                (model, TL_coef, data_norms, y_hat_fi, err_fi) = nn_comp_3_train(A, Bt, B_dot, x_fi, y, no_norm;
                                                                                 model_type    = model_type,
                                                                                 norm_type_x   = norm_type_x,
                                                                                 norm_type_y   = norm_type_y,
                                                                                 TL_coef       = TL_coef,
        ...
        # train model
        if model_type in [:m1]
            (model, data_norms, y_hat, err) = nn_comp_1_train(x, y, no_norm;
                                                              norm_type_x = norm_type_x,
                                                              norm_type_y = norm_type_y,
                                                              η_adam      = η_adam,
                                                              epoch_adam  = epoch_adam,
                                                              ...
                                                              silent      = silent)
        elseif model_type in [:m2a, :m2b, :m2c, :m2d]
            (model, TL_coef, data_norms, y_hat, err) = nn_comp_2_train(A, x, y, no_norm;
                                                                       model_type  = model_type,
                                                                       norm_type_A = norm_type_A,
                                                                       norm_type_x = norm_type_x,
                                                                       norm_type_y = norm_type_y,
                                                                       ...
                                                                       silent      = silent)
        elseif model_type in [:m3tl, :m3s, :m3v, :m3sc, :m3vc, :m3w, :m3tf]
            (model, TL_coef, data_norms, y_hat, err) = nn_comp_3_train(A, Bt, B_dot, x, y, no_norm;
                                                                       model_type    = model_type,
                                                                       norm_type_x   = norm_type_x,
                                                                       norm_type_y   = norm_type_y,
                                                                       TL_coef       = TL_coef,
        ...
        elseif model_type in [:TL, :mod_TL, :map_TL]
            ...
            (model, data_norms, y_hat, err) = linear_fit(A, y;
                                                         trim        = trim,
                                                         λ           = λ_TL,
...
function comp_train(comp_params::CompParams, xyz_vec::Vector, ind_vec::Vector,
    ...
    for (xyz, ind) in zip(xyz_vec[2:end], ind_vec[2:end])
        ...
        model_type in [:TL, :mod_TL] && (y_no_bpf = vcat(y_no_bpf,
                                                         get_y(xyz, ind, map_val;
                                                               y_type      = :d,
                                                               use_mag     = use_mag,
                                                               sub_diurnal = sub_diurnal,
    ...
    if drop_fi
        ...
        elseif model_type in [:TL, :mod_TL, :map_TL]
            ...
            (model, data_norms, y_hat, err) = linear_fit(A, y;
                                                         ...
                                                         silent      = silent)
            if model_type in [:TL, :mod_TL]
                (A_no_bpf, _, y_no_bpf, _, _, _) = get_Axy(lines, df_line, df_flight, df_map,
                                                           features_setup;
                                                           features_no_norm = features_no_norm,
                                                           y_type           = :d,
                                                           use_mag          = use_mag,
    ...
    if drop_fi | perm_fi
        ...
        for i in axes(x, 2)
            ...
            open(fi_csv, "a") do file
                return writedlm(file, zip(i, std(err_fi)), ',')
            end
        end
...
function comp_m3_test(comp_params::NNCompParams, lines,
    ...
    (TL_aircraft, TL_perm, TL_induced, TL_eddy) = get_TL_aircraft_vec(B_vec, B_vec_dot, TL_coef_p, TL_coef_i, TL_coef_e;
                                                                      return_parts=true)
...
function comp_train_test(comp_params::CompParams,
    ...
    (comp_params, y_train, y_train_hat, err_train, features) = comp_train(comp_params, xyz_train, ind_train, mapS_train;
                                                                          temp_params = temp_params,
                                                                          silent      = silent)

    (y_test, y_test_hat, err_test, _) = comp_test(comp_params, xyz_test, ind_test, mapS_test;
                                                  temp_params = temp_params,
                                                  silent      = silent)
...
function comp_train_test(comp_params::CompParams, lines_train, lines_test,
                         ...
                         temp_params::TempParams = TempParams(),
                         silent::Bool            = false)
    (comp_params, y_train, y_train_hat, err_train, features) = comp_train(comp_params, lines_train, df_line, df_flight,
                                                                          df_map;
                                                                          temp_params = temp_params,
                                                                          silent      = silent)
