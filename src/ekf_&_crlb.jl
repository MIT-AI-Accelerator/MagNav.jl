...
function ekf(ins::INS, meas, itp_mapS;
             ...
             der_mapS   = map_itp(zeros(2, 2), [-pi, pi], [-pi/2, pi/2]),
             map_alt    = 0)
    return ekf(ins.lat, ins.lon, ins.alt, ins.vn, ins.ve, ins.vd, ins.fn, ins.fe, ins.fd,
...
function (ekf_rt::EKF_RT)(ins::INS, meas, itp_mapS;
    ...
    return ekf_rt(ins.lat[1], ins.lon[1], ins.alt[1], ins.vn[1], ins.ve[1], ins.vd[1],
...
function crlb(traj::Traj, itp_mapS;
              ...
              date       = get_years(2020, 185),
              core::Bool = false)
    return crlb(traj.lat, traj.lon, traj.alt, traj.vn, traj.ve, traj.vd,
