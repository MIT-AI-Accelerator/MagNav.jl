...
@testset "get_nn_m tests" begin
    ...
    @test get_nn_m(1, 1; hidden=[1], model_type=:m3tf, tf_layer_type=:prelayer, tf_norm_type=:layer, N_tf_head=1) isa
          Chain
    @test get_nn_m(1, 1; hidden=[1], model_type=:m3tf, tf_layer_type=:postlayer, tf_norm_type=:layer, N_tf_head=1) isa
          Chain
    @test get_nn_m(1, 1; hidden=[1, 1], model_type=:m3tf, tf_layer_type=:prelayer, tf_norm_type=:batch, N_tf_head=1) isa
          Chain
    @test get_nn_m(1, 1; hidden=[1, 1], model_type=:m3tf, tf_layer_type=:postlayer, tf_norm_type=:none, N_tf_head=1) isa
          Chain
...
@testset "project_body_field_to_2d_igrf tests" begin
    ...
    north_vec_body = [Cnb[:, :, i]'*north_vec_nav for i in 1:size(Cnb)[3]]
    n2ds = [project_body_field_to_2d_igrf(north_vec_body[i], north_vec_nav, Cnb[:, :, i])
            for i in 1:size(Cnb)[3]]
    @test all(map(v -> v ≈ [1.0, 0.0], n2ds))

    east_vec_nav  = vec([0.0, 1.0, 0.0])
    east_vec_body = [Cnb[:, :, i]'*east_vec_nav for i in 1:size(Cnb)[3]]
    ...
    west_vec_body = [Cnb[:, :, i]'*west_vec_nav for i in 1:size(Cnb)[3]]
    ...
    south_vec_body = [Cnb[:, :, i]'*south_vec_nav for i in 1:size(Cnb)[3]]
...
(TL_aircraft, TL_perm, TL_induced, TL_eddy) = MagNav.get_TL_aircraft_vec(B_vec', B_vec_dot', TL_coef_p, TL_coef_i,
                                                                         TL_coef_e;
                                                                         return_parts=true)
y_nn = zeros(3, 50)
y = y_hat = zeros(50)
