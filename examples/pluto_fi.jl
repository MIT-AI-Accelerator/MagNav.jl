...
begin
    (comp_params, _, _, _, feats) = comp_train(comp_params_init, lines_train, df_all, df_flight, df_map)
...
begin
    (_, x_train, y_train, _, _, _) = MagNav.get_Axy(lines_train, df_all, df_flight, df_map, features;
                                                    use_mag=use_mag, use_vec=use_vec, terms=terms)
    (_, x_test, y_test, _, _, _) = MagNav.get_Axy(lines_test, df_nav, df_flight, df_map, features;
                                                  use_mag=use_mag, use_vec=use_vec, terms=terms)
