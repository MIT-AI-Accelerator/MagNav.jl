...
(comp_params_1, _, _, _, feats) = comp_train(comp_params_1_init, lines_train, df_all, df_flight, df_map);
...
(comp_params_2, _, _, _, _) = comp_train(comp_params_2_init, TL_xyz, TL_ind);
...
begin
    (_, x_train, y_train, _, _, l_segs_train) = MagNav.get_Axy(lines_train, df_all, df_flight, df_map, features;
                                                               use_mag=use_mag, use_vec=use_vec, terms=terms)
    (_, x_test, y_test, _, _, l_segs_test) = MagNav.get_Axy(lines_test, df_nav, df_flight, df_map, features;
                                                            use_mag=use_mag, use_vec=use_vec, terms=terms)
...
begin
    ...
    (model, data_norms, y_train_hat, _) = elasticnet_fit(x_train, y_train, α; l_segs=l_segs_train)
